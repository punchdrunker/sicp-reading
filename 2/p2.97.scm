...
(define (install-sparse-terms-package)
  ...
  (define (reduce-terms n d)
    (let* ((gcd-nd (reduce (gcd-terms n d)))
           (nd-factor (integerising-factor n d))
           ; div terms returns 2 termlists, but the remainder must be 0
           ; so just use the divisor which is the first termlist
           (nn (car (div-terms (mul-integer-terms nd-factor n) gcd-nd)))
           (dd (car (div-terms (mul-integer-terms nd-factor d) gcd-nd)))
           (nndd-list (append (map coeff nn) (map coeff dd)))
...
(define (install-rational-package)
  ...
  (define (rational->real r) (make-real (exact->inexact (/ (numer r) (denom r))))) 
  (define (project r) 
  ...
  (put 'sq-root    '(rational)           (lambda (x)   (make-real (sqrt (/ (numer x) (denom x))))))
  ...
  (put 'arctan     '(rational rational)  (lambda (x y) (make-real (atan x y))))
