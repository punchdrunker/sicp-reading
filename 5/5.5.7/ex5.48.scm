; 問題 5.48
;
; 本節で実装したcompile-and-goのインターフェースは, (評価器計算機を起動した時に)翻訳系は一回しか呼び出せず扱い難い.
; 翻訳系-解釈系のインターフェースを拡張し, 積極制御評価器から次のように呼び出せるcompile-and-run基本手続きを用意せよ:
;;;; EC-Eval input:
;(compile-and-run
;  '(define (factorial n)
;     (if (= n 1)
...
(define (setup-environment-extended)
  (extend-environment
...
(define (compile-and-go expression)
  (let ((instructions
          (assemble (statements
                    ...
                    eceval)))
    (set! the-global-environment (setup-environment-extended))
...
(define eceval-operations
  (list (list 'self-evaluating? self-evaluating?)
        ...
        ;; compile-and-run?, compile-and-runを追加
